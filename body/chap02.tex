...
\subsection{基于锚框的单阶段时空动作检测算法}

介绍MOC和YOWO等基于锚框的单阶段时空动作检测方法。

\subsection{基于查询的时空动作检测算法}

\subsubsection{DETR设计概述}
...
本文使用的数据集有JHMDB51-24、UCF101-24、Multisports，使用的评价指标包括性能评价指标
...
JHMDB51-24是一个广泛用于时空动作检测任务的基准数据集，由HMDB51数据集筛选出的928个
视频片段组成。该数据集包含21个动作类别，主要涉及单人日常行为与运动（如“梳头”、“接球”、“跑”等）。
JHMDB51-24的：其边界框真值 (Ground Truth Bounding Boxes) 并非由人工直接绘制，
而是基于每帧标注的15个2D人体关节位置生成的分割掩码 (Puppet Mask) 推导而来，
因此具有极高的人体贴合度。

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/ucf101_2-5.png}
    \caption{JHMDB51-21数据集样例}\label{Fig2-5}
\end{figure}
